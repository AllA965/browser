...
namespace MiniWorldBrowser.Services
{
    ...
    public class MediaDownloadService
    {
        ...
        /// <summary>
        /// 尝试从当前页面提取实际的视频播放地址（针对抖音等 Feed 流页面）
        /// </summary>
        public async Task<string> GetEffectiveVideoUrlAsync(CoreWebView2 webView, string currentUrl)
        {
            System.Diagnostics.Debug.WriteLine($"GetEffectiveVideoUrlAsync: {currentUrl}");
            
            // 针对抖音进行深度数据提取 (方案 A: 直接抓取页面 JSON 数据)
            if (currentUrl.Contains("douyin.com"))
            {
                ...
                const string script = @"(async function() {
                    try {
                        ...
                        // 1. 获取所有可能的视频数据源 (RENDER_DATA 或全局变量)
                        const getRawData = () => {
                        ...
                        // --- Scheme B: Fetch Official API with Signature ---
                        if (activeId) {
                            try {
                                const aid = 6383;
                        ...
                        // --- Fallback to Scheme A: RENDER_DATA ---
                        let vData = null;
            ...
            return JsonSerializer.Serialize(new { type = "url", url = currentUrl });
        }
